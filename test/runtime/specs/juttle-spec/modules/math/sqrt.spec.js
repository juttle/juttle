...
describe('The Math.sqrt function', function() {

    it('Produces an error when passed an argument of incorrect type', function() {
        var program = 'emit -from Date.new(0) -limit 1 | put result = Math.sqrt(null) | view result';
        var moduleResolver = juttle_test_utils.module_resolver({});

        return check_juttle({ program: program, moduleResolver: moduleResolver })
            .then(function(res) {
                expect(res.warnings[0]).to.include("Invalid argument type for \"Math.sqrt\": expected number, received null.");
            res.prog.deactivate();
            });
    });
}); // describe block 
