...
var GraphCompiler = CodeGenerator.extend({
    ...
    gen_now: function(now) {
        this.emit('program.now = ' + 'new JuttleMoment("' + now + '");');
    },
    ...
    gen_Sink: function(ast) {
        ...
        // and AST.
        var proc = this.gen_proc(ast, 'clientsink', '{name: "'+ast.name+'"}');
    ...
    gen_CalendarExpression: function(ast) {
        if (ast.direction === 'up') {
            return 'JuttleMoment.endOf('+ this.gen_expr(ast.expression) +', "'+ ast.unit +'")';
        } else {
            return 'JuttleMoment.startOf('+ this.gen_expr(ast.expression) +', "'+ ast.unit +'")';
        }
    },
