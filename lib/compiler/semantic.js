...
var SemanticPass = Base.extend({
    ...
    function_name: function(ast) {
        return ast.name.type !== 'PropertyAccess'
            ? ast.name.name
            : ast.name.base.name + '.' + ast.name.name.value;
    },
    check_arg_count: function(name, type, actual_count, expected_count, location) {
        ...
        if (actual_count < min_count || actual_count > max_count) {
            if (min_count === max_count) {
                if (min_count === 1) {
                    throw errors.compileError('RT-WRONG-ARG-COUNT-SINGULAR', {
                        type: type,
                        name: name,
                    ...
                    throw errors.compileError('RT-WRONG-ARG-COUNT-PLURAL', {
                        type: type,
                        name: name,
                ...
                throw errors.compileError('RT-WRONG-ARG-COUNT-RANGE', {
                    type: type,
                    name: name,
                    min_count: min_count,
                    max_count: max_count,
    ...
    check_export: function(ast) {
        if (ast.export  && !this.can_export()) {
            throw errors.compileError('RT-NON-TOPLEVEL-EXPORT', {
    ...
    check_reducer_toplevel: function(ast, thing) {
        if (this.context() === 'reducer') {
            throw errors.compileError('RT-REDUCER-TOPLEVEL-STATEMENT', {
                thing: thing,
                location: ast.location
            });
        }
    ...
    sa_SubDef: function(ast) {
        ...
        if (this.context() === 'top') {
            ...
            if (!has_graph) {
                throw errors.compileError('RT-PROGRAM-WITHOUT-FLOWGRAPH', {
                    location: ast.location
                });
            }
        }
    ...
    sa_args: function(args) {
        ...
        for (var k = 0; k < args.length; k++) {
            ...
            if (seen_optional && !is_optional) {
                throw errors.compileError('RT-PARAM-ORDER', {
    ...
    compute_arg_count: function(args) {
        var counts = _.countBy(args, function(arg) {
            return arg.default ? 'optional' : 'required';
        });

        return [counts.required, counts.required + counts.optional];
    },
            ...
            if (node.type === 'FunctionDef') {
                if ((node.name === 'update' || node.name === 'result' || node.name === 'expire' || node.name === 'reset')) {
                    if (node.args.length > 0) {
                        throw errors.compileError('RT-REDUCER-METHOD-ARGS', {
                            name: node.name,
    ...
    sa_InputStatement: function(ast, opts) {
        ...
        ast.options = _.map(ast.options, function(opt) {
            ...
            if (! ret.expr.d) {
                throw errors.compileError('RT-BAD-INPUT-OPTION', {
                    input: ast.name,
                    option: opt.id,
    ...
    sa_ConstStatement: function(ast, opts) {
        ...
        for (k = 0; k < decls.length; ++k) {
            if (!decls[k].expr) {
                throw errors.compileError('RT-UNINITIALIZED-CONST', {
                    name: decls[k].name,
                    location: ast.location
                });
            }
    ...
    sa_ImportStatement: function(ast, functions) {
        ...
        if (this.context() !== 'module' && this.context() !== 'main') {
            throw errors.compileError('RT-NON-TOPLEVEL-IMPORT', {
                context: this.context(),
                location: ast.location
            });
        }
        if (_.contains(this.import_path, modulename)) {
            ...
            throw errors.compileError('RT-CYCLIC-IMPORT', {
    ...
    sa_sequence_proc: function(ast) {
        ...
        if (ast.filters.length < 2) {
            throw errors.compileError('RT-PROC-NEEDS-ARG', {
                name: 'sequence',
                location: ast.location
            });
        }
    ...
    sa_TodayLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.value = this.now.startOf('day').valueOf();
        ast.d = true;
        return ast;
    },
    sa_YesterdayLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.value = this.now.startOf("day").subtract(JuttleMoment.duration(1, "day")).valueOf();
        ast.d = true;
        return ast;
    },
    sa_TomorrowLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.value = this.now.startOf("day").add(JuttleMoment.duration(1, "day")).valueOf();
        ast.d = true;
        return ast;
    },
    sa_ForeverLiteral: function(ast) {
        ast.type = 'DurationConstant';
        ast.value = JuttleMoment.duration(Infinity).valueOf();
        ast.d = true;
        return ast;
    },
    sa_ISODateLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.d = true;
        return ast;
    },
    sa_UnixTimeLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.value = new JuttleMoment(ast.value).valueOf();
        ast.d = true;
        return ast;
    },
    sa_BeginningLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.value = new JuttleMoment(-Infinity).valueOf();
        ast.d = true;
        return ast;
    },
    sa_EndLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.value = new JuttleMoment(Infinity).valueOf();
        ast.d = true;
        return ast;
    },
    sa_NowLiteral: function(ast) {
        ast.type = 'MomentConstant';
        ast.value = this.now.valueOf();
        ast.d = true;
        return ast;
    },
    sa_MomentDuration: function(ast) {
        ast.type = 'DurationConstant';
        if (ast.unit) {
            ast.value = JuttleMoment.duration(ast.value, ast.unit).valueOf();
            delete ast.unit;
        } else {
            ast.value = JuttleMoment.duration(ast.value).valueOf();
        }
        ast.d = true;
        return ast;
    },
    ...
    sa_CalendarExpression: function(ast) {
        ast.expression = this.sa_expr(ast.expression);
        ast.d = true;
        return ast;
    },
    ...
    sa_ReduceProc: function(ast) {
        ...
        reified.reducers.forEach(function(reducer) {
            if (reducer.name === 'delta') {
                throw errors.compileError('RT-REDUCE-DELTA-ERROR', {
                    location: ast.location
                });
            }
        });
    ...
    sa_FunctionProc: function(ast) {
        ...
        if (ast.op.type === "PropertyAccess") {
            sub = this.scope.lookup_module_sub(ast.op.base.name, ast.op.name.value);
            if (sub === undefined) {
                throw errors.compileError('RT-NOT-EXPORTED', {
                    thing: 'sub',
                    name: ast.op.name.value,
                    module: ast.op.base.name,
                    location: ast.location
                });
            }
            ast.op.base.symbol = this.scope.get(ast.op.base.name);
        } else {
            sub = this.scope.get(ast.op.name);
            if (!sub) {
                throw errors.compileError('RT-NO-SUCH-SUB', {
                    name: ast.op.name,
                    location: ast.location
                });
            }
            if (sub.type !== 'sub') {
                throw errors.compileError('RT-NOT-A-SUB', {
                    name: ast.op.name,
                    location: ast.location
                });
            }
    ...
    sa_assignment_lhs: function(ast, opts) {
        ...
        case 'Variable':
            if (!this.scope.lookup_variable(ast.name)) {
                throw errors.compileError('RT-UNDEFINED-VARIABLE', {
                    name: ast.name,
                    location: ast.location
                });
            }
            if (!this.scope.is_mutable(ast.name, opts, this.context())) {
                throw errors.compileError('RT-VARIABLE-NOT-ASSIGNABLE', {
                    name: ast.name,
                    location: ast.location
                });
            }
        ...
        case 'PropertyAccess':
            ...
            return this.sa_PropertyAccess(ast, opts);
    ...
    sa_expr: function(ast, opts) {
        ...
        case 'TodayLiteral':
        case 'YesterdayLiteral':
        case 'TomorrowLiteral':
        case 'ForeverLiteral':
        case 'ISODateLiteral':
        case 'UnixTimeLiteral':
        case 'NowLiteral':
        case 'BeginningLiteral':
        case 'EndLiteral':
        case 'MomentDuration':
    ...
    sa_PostfixExpression: function(ast, opts) {
        ...
        case 'PropertyAccess':
            ...
            return ast;
        default:
            throw errors.compileError('RT-INVALID-POSTFIX-LHS', {
                location: ast.location
            });
        }
    },
    sa_FunctionCall: function(ast, opts) {
        ...
        if (ast.name.type === 'PropertyAccess') {
            args = ast.arguments;

            funcInfo = this.scope.lookup_module_reducer(ast.name.base.name, ast.name.name.value);
            if (funcInfo) {
                // XXX opts.reducers implies reducer context
                if (!opts.reducers) {
                    throw errors.compileError('RT-INVALID-REDUCER-CALL', {
                        name: ast.name.base.name + '.' + ast.name.name.value,
                        location: ast.location
                    });
                }
                opts.reducers.push({
                    module: ast.name.base.name,
                    name:ast.name.name.value,
                    args:args,
                    index: opts.index
                });
                ast.type = 'ReducerCall';
                ast.reducer_call_index = opts.reducers.length - 1;
                ast.name.base.symbol = this.scope.get(ast.name.base.name);
                ...
                funcInfo = this.scope.lookup_module_func(ast.name.base.name, ast.name.name.value);
        ...
        if (is_builtin_reducer(fname) || (this.scope.get(fname) && this.scope.get(fname).type === 'reducer')) {
            ...
            if (!funcInfo) {
                throw errors.compileError('RT-NO-SUCH-FUNCTION', {
                    name: fname,
                    location: ast.location
                });
            }
            if (funcInfo.type !== 'function') {
                throw errors.compileError('RT-NOT-A-FUNCTION', {
                    name: fname,
                    location: ast.location
                });
            }
    ...
    sa_UnaryExpression: function(ast, opts) {
        ...
        if (op === '++' || op === '--') {
            name = ast.expression.type === 'Variable' ?
                ast.expression.name : ast.expression.base.name;
            if (!this.scope.is_mutable(name, opts, this.context())) {
                throw errors.compileError('RT-INVALID-PREFIX-USE', {
                    operator: op,
                    variable: name,
                    location: ast.location
                });
    ...
    sa_Variable: function(ast, opts) {
        ...
        if (varInfo) {
            ...
            switch (opts.coerce_var) {
                ...
                default:
                    throw errors.compileError('RT-UNDEFINED', {
                        name: ast.name,
                        location: ast.location
                    });
            }
    ...
    sa_PropertyAccess: function(ast, opts) {
        var varInfo;
        if (!ast.computed) {
            varInfo = this.scope.lookup_module_variable(ast.base.name, ast.name.value);
            if (varInfo === undefined) {
                throw errors.compileError('RT-NOT-EXPORTED', {
                    thing: 'variable',
                    name: ast.name.value,
                    module: ast.base.name,
                    location: ast.location
                });
            }

            ast.base = this.sa_expr(ast.base, opts);

            ast.uname = varInfo.name;
            ast.symbol = this.scope.get(ast.base.name).exports[ast.name.value];
            ast.d = true;
        }
        else {
            ast.base = this.sa_expr(ast.base, opts);
            ast.name = this.sa_expr(ast.name, opts);
            ast.d = ast.base.d && ast.name.d;
