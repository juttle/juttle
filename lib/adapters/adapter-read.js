...
var juttle_utils = require('../runtime/juttle-utils');
...
class AdapterRead {
    ...
    static commonOptions() { return ['from', 'to', 'last', 'timeField', 'every', 'lag']; }
    ...
    // into JuttleMoment.
    parseTime(points, options) {
        return juttle_utils.toNativeTime(points, options, this);
    }
    ...
    // Helper to obtain a throttled function to emit warnings.
    throttledWarning(code, info, interval) {
        interval = interval || 1000;
        info = info || {};
        return _.throttle(() => {
            this.trigger('warning', this.runtimeError(code, info));
        }, interval);
    }
