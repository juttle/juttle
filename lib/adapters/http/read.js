...
class Iterator {
    constructor(adapter, res) {
        ...
        this.parser.parseStream(res, (points) => {
            points = this.process(res, points);
...
class ReadHTTP extends AdapterRead {
    ...
    allowedOptions() {
        return ['url', 'method', 'headers', 'body', 'timeField', 'includeHeaders', 'rootPath', 'format'];
    }
    ...
    fetch() {
        return new Promise((resolve, reject) => {
            ...
            .on('error', (err) => {
                reject(new errors.runtimeError('RT-INTERNAL-ERROR', {
                    error: err.toString()
                }));
            })
            .on('response', (res) => {
