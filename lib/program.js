...
var Program = Base.extend({
    ...
    // returns a list of the sinks of this program, formatted for use by the app
    get_client_sinks: function() {
        var allSinks = this.get_sinks();
        return _.filter(allSinks, function(sink) {
            return sink.procName === 'clientsink';
        })
        .map(function(sink) {
            return {name: sink.name, channel: sink.channel, options: sink.options, location: sink.location};
        });
    },
