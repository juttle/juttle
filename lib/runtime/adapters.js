...
function getInstances(type, conf) {
    ...
    if (conf.instances) {
        if (!_.isArray(conf.instances)) {
            throw new Error(`instances key in configuation for adapter ${type} must be an array.`);
        }
    ...
    if (instances.length > 1) {
        _.each(instances, (instance) => {
            if (!instance.id) {
                throw new Error(`One or more configuation instances for adapter ${type} is missing an id.`);
            }
        });
    }
...
function loadAdapter(type, location) {
    ...
    try {
        var modulePath = adapterModulePath(type, adapterConf);
