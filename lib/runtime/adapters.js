...
function getInstances(type, conf) {
    var instances;
    
    if (conf.instances) {
        if (!_.isArray(conf.instances)) {
            throw new Error(`instances key in configuration for adapter ${type} must be an array.`);
        }
        instances = conf.instances;
    } else {
        instances = _.isArray(conf) ? conf : [_.omit(conf, 'path')];
    }
    
    if (instances.length > 1) {
        _.each(instances, (instance) => {
            if (!instance.id) {
                throw new Error(`One or more configuration instances for adapter ${type} is missing an id.`);
            }
        });
    }
...
function loadAdapter(type, location) {
    var adapterConf = adapterConfig[type];
    
    try {
        var modulePath = AdapterHelper.adapterModulePath(type, adapterConf);
        var instances = getInstances(type, adapterConf);
        
        global.JuttleAdapterAPI = JuttleAdapterAPI;

        var start = new Date();

        if (!adapterConf.builtin) {
...
function list() {
    ...
    _.each(adapterConfig, function(config, adapter) {
        var modulePath = AdapterHelper.adapterModulePath(adapter, config);
