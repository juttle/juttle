...
var ops = {
    ...
    match: function(left, right) {
        if (values.isString(right)) {
            if (values.isString(left)) {
                return gex(right).on(left) !== null ? true : false;
            } else {
                return false;
            }
    ...
    lt: function(left, right) {
        if (values.isNumber(left) && values.isNumber(right)) {
            ...
            return JuttleMoment.lt(left, right);
        } else if (values.isDuration(left) && values.isDuration(right)) {
            return JuttleMoment.lt(left, right);
        } else if (values.isArray(left) && values.isArray(right)) {
            if (left.length !== right.length) {
                return left.length < right.length;
            } else {
                for (var i = 0; i < left.length; i++) {
                    if (this.lt(left[i], right[i])) {
                        return true;
                    } else if (this.gt(left[i], right[i])) {
                        return false;
                    }
                }
    ...
    gt: function(left, right) {
        if (values.isNumber(left) && values.isNumber(right)) {
            ...
            return JuttleMoment.gt(left, right);
        } else if (values.isDuration(left) && values.isDuration(right)) {
            return JuttleMoment.gt(left, right);
        } else if (values.isArray(left) && values.isArray(right)) {
            if (left.length !== right.length) {
                ...
                for (var i = 0; i < left.length; i++) {
                    if (this.gt(left[i], right[i])) {
                        return true;
                    } else if (this.lt(left[i], right[i])) {
                        return false;
                    }
                }
    ...
    lte: function(left, right) {
        if (values.isNumber(left) && values.isNumber(right)) {
            ...
            return JuttleMoment.lte(left, right);
        } else if (values.isDuration(left) && values.isDuration(right)) {
            return JuttleMoment.lte(left, right);
        } else if (values.isArray(left) && values.isArray(right)) {
            return values.equal(left, right) || this.lt(left, right);
        } else {
            throw errors.typeErrorBinary('<=', left, right);
        }
    },
    ...
    gte: function(left, right) {
        if (values.isNumber(left) && values.isNumber(right)) {
            ...
            return JuttleMoment.gte(left, right);
        } else if (values.isDuration(left) && values.isDuration(right)) {
            return JuttleMoment.gte(left, right);
        } else if (values.isArray(left) && values.isArray(right)) {
            return values.equal(left, right) || this.gt(left, right);
        } else {
            throw errors.typeErrorBinary('>=', left, right);
        }
    },
    ...
    pget: function(point, index) {
        if (Object.prototype.hasOwnProperty.call(point, index)) {
            return point[index];
        } else {
            return null;
        }
