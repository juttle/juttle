...
class join extends base {
    ...
    queue_tpoint(tpoint, input) {
        ...
        if (tpoint.eof) {
            ...
            input.last().interval = tpoint.interval;
    ...
    emit_result(points) {
        ...
        // Mark interval will be the distance between group start and complete times.
        var leader = this.batched_leader() ;
        var join_time = this.outer ? this.ins[this.outer - 1].live_time() : this.max_live_time();
        var interval;

        if (leader) {
            var live_leader = leader.live();
            interval = live_leader.interval;
            interval = interval || live_leader.complete_time.subtract(live_leader.start_time);
        }

        if (leader) {
            this.maybe_emit_mark(leader.live().start_time, interval);
        }
        if (points.length) {
            this.process(points);
        ...
        if (leader) {
            this.maybe_emit_mark(leader.live().complete_time, interval);
        }
    }
    maybe_emit_mark(time, interval) {
