...
var join = base.extend({
    initialize: function(options) {
        ...
        if (options.zip !== undefined && options.nearest !== undefined) {
            throw this.compile_error('RT-JOIN-ZIP-NEAREST-ERROR');
        }
        ...
        if (options.zip !== undefined) {
            if (options.zip.duration) {
                ...
                throw this.compile_error('RT-JOIN-ZIP-ERROR');
            }
        ...
        if (this.maxoffset && !this.maxoffset.duration) {
            throw this.compile_error('RT-INVALID-OFFSET-ERROR');
        }
        var unknown = _.without(_.keys(options), 'zip', 'nearest', 'once', 'outer', 'table');
        if (unknown.length > 0) {
        ...
        if (options.outer !== undefined) { // outer join, preserving this input
            ...
            if (!_.isNumber(this.outer)) {
                throw this.compile_error('RT-JOIN-INPUTNUM-ERROR', {
                    option: 'outer'
                });
            }
        } else {
    ...
    check_settings: function() {
        ...
        if (_.isNumber(this.outer) && (this.outer < 1 || this.outer > this.ins.length)) {
            throw this.runtime_error('RT-JOIN-INPUTNUM-ERROR', {
                option: 'outer'
            });
        }
        if (this.table) {
            if (this.table.length === this.ins.length) {
                throw this.runtime_error('RT-JOIN-ALL-TABLES-ERROR');
            }
            for (var i = 0; i < this.table.length; i++) {
                ...
                if (!_.isNumber(idx) || idx < 1 || idx > this.ins.length) {
                    ...
                    throw this.runtime_error('RT-JOIN-TABLE-OUTER-ERROR');
                }
    ...
    witness_timestamp: function(tpoint, input) {
        ...
        var last = input.last();
        var last_time = last && last[0].time;
        ...
        if (input.batched) {
            ...
            if (lastlast && !tpoint.eof) {
                lastlast.next_time = last.batch_end;
            } else if (lastlast && tpoint.eof) {
                lastlast.next_time = tpoint.time; // skip over final mark to eof (Infinity)
            }
        } else if (last) {
            if (!input.table && tpoint.time && tpoint.time.gt(last_time) && !this.once) {
    ...
    queue_tpoint: function(tpoint, input) {
        ...
        input.last().push(tpoint); // add tpoint to newest group
        if (input.table) {
            var last = input.last();
    ...
    advance_input: function(tpoint, input) {
        ...
        if (tpoint.mark && input.table) {
            throw this.runtime_error('RT-JOIN-TABLE-BATCHED-ERROR');
