...
// clients
Juttle.proc.clientsink = Juttle.proc.publish.extend({
    initialize: function(options, params) {
    ...
    // When client sinks get points/marks/ticks, they emit events on
    // the related program object. Program users will subscribe for
    // these events to receive the information from the sinks.
    //
    ...
    mark: function(time) {
        ...
        this.program.trigger("sink:mark", {channel: this.channel, time: time});
    },
    tick: function(time) {
        Juttle.proc.publish.prototype.tick.call(this, time);
        this.program.trigger("sink:tick", {channel: this.channel, time: time});
    },
    eof: function() {
        Juttle.proc.publish.prototype.eof.call(this);
        this.program.trigger("sink:eof", {channel: this.channel});
        this.done();
    },
    process: function(points) {
        Juttle.proc.publish.prototype.process.call(this, points);
        this.program.trigger("sink:points", {channel: this.channel, points: points});
    }
