...
var source = base.extend({
    ...
    // Indication that the subclass has completed reading up to the given time.
    // Check if we're all done, and emit any needed ticks.
    advance: function(endTime) {
        if (endTime.eq(this.to)) {
            this.logger.debug('advance: all done');
            this.emit_eof();
            return;
        }

        while (this.nextTick.lt(endTime)) {
        ...
        // XXX this should only be done when the scheduler requests it after the
        // buffer is drained.
        this.schedule();
    }
}, {
