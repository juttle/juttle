...
var Duration = require('../../../moment').JuttleMoment.duration;
...
function make_source(options, filter, start, end, every, now, location) {
    ...
    if (config.dos) {
        ...
        switch (config.dos_router) {
            ...
            config.host_names, config.dos_dur, true, location);
                break;
            default:
                throw juttleErrors.compileError('RT-UNKNOWN-DOS-ROUTER', {
    ...
    function make_host_events(host, funcs, location) {
        ...
        var warn_too_many = _.once(function (errors) {
            if (errors) {
                errors.push(new juttleErrors.runtimeError('RT-TOO-MANY-LOGS-WARNING', {
                    host: host.name,
                    location: location
                }));
            }
