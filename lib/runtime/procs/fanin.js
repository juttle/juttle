...
class Input {
    ...
    //
    consume(points, from) {
        this.shifts = this.shifts || 0;
        this.stats.points_in += points.length;

        console.log(`${from.params.pname} points: ${JSON.stringify(points)}`);

        _.each(this.ins, function(input) {
            console.log(`${input.from.params.pname} qlen: ${input.length()} ${input.first() ? input.first().time.valueOf() : '-'} ${input.last() ? input.last().time.valueOf() : '-'}`);
        });

        if (from.params.pname === 'p0' && this.in_from(from).length() == 3) {
            if (this.shifts === 1) {
                console.log(this.in_from(from).toArray());
            }
            this.shifts++;
        }
