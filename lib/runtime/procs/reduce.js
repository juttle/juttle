...
class reduce_base extends periodic_fanin {
    ...
    witness_timestamp(time) {
        ...
        if (this.to && this.to.lte(time)) {
            ...
            // The last interval is possibly shorter
            if (this.last_epoch) {
                interval = this.to.subtract(this.last_epoch);
            }

            var out = this.advance_epoch(this.to, interval);
    ...
    advance_epoch(epoch, interval) {
        ...
        if (!epoch || (this.good_epoch(epoch) && this.last_epoch.lt(epoch))) {
            ...
            this.groups.apply(function(keyID) {
                ...
                // ...and interval info from incoming marks
                if (interval) {
...
class reduce_batch extends reduce_base {
    ...
    // output mark after any generated points for this batch.
    ...
    mark(mark, from) {
        if (this.has_first_mark) {
            var out = this.advance_epoch(mark.time, mark.interval);
            this.emit(out, from);
            this.emit_mark(mark, from);
        } else {
            this.emit_mark(_.omit(mark, 'interval'), from);
...
class reduce_every extends reduce_base {
    ...
    eof(from) {
        ...
        if (!this.batch_size && !this.after_first_epoch) {
            var out = this.advance_epoch(this.next_epoch, this.interval);
            this.emit(out);
        }
        super.eof(from);
    }
