...
var _reduce = __reduce.extend({
    initialize: function(options, params) {
        var unknown = _.without(_.keys(options), 'acc', 'every', 'forget', 'from', 'groupby', 'on', 'over', 'reset', 'to', 'no_groupby_warning');
        if (unknown.length > 0) {
...
function reduce(options, params, pname, location, program) {
    return options.every? new _ereduce(options, params, pname, location, program) : new _reduce(options, params, pname, location, program);
}
