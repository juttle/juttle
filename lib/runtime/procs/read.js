    ...
    run() {
        ...
        .then((result) => {
            ...
            if (result.points && result.points.length !== 0) {
                ...
                //XXX/bkutil: will we want to keep named outputs, convert optimizations
                // to use them (support optimizing non-linear topologies)?
            ...
            if (result.eof || result.readEnd) {
                ...
                if (result.eof || result.readEnd.isEnd() || (this.to && result.readEnd.eq(this.to))) {
                    ...
                    this.flowControl.emit({ emitAll: true });
                    this.emit_eof();
                    return;
                }
        ...
        .catch((err) => {
            ...
            if (! (err instanceof errors.JuttleError)) {
                err = new errors.runtimeError('INTERNAL-ERROR', {
                    error: err.toString()
                });
            }
            this.trigger('error', err);
