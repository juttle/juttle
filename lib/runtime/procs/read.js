...
var Read = source.extend({
    ...
    initialize: function(options, params) {
        ...
        var timeRequired = adapter.read.timeRequired;
        if (timeRequired === undefined) {
            throw new Error('adapter must define a timeRequired property');
        }
        this.handleTimeOptions(options);
        this.setDefaultTimeRange(timeRequired);
        ...
        .then((result) => {
            ...
            if (result.readEnd) {
                ...
                if (result.readEnd.eq(this.readTo)) {
                    ...
                    this.readTo = this.readFrom.add(JuttleMoment.min(this.readEvery, this.tickEvery));
                } else {
