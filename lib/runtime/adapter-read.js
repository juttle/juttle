...
class AdapterRead {
    ...
    //
    // The boolean flag `timedSource` must be supplied to indicate thether
    // the adapter requires time bounds or not.
    constructor(options, params, timedSource) {
        if (timedSource === undefined) {
            throw new Error('adapter read must supply timedSource option');
        }
